...
async function _handleKEMAnalyze(msg) {
  ...
  try {
    ...
    if (!principalFrameArt) throw new Error('principal_frame artifact required for KEM');
    if (!flowFieldArt)      throw new Error('flow_field artifact required for KEM');
    if (!motionMapsArt)     throw new Error('motion_maps artifact required for KEM');
    ...
    const [
      ...
      _loadArtifact(api, artifactKeys.narrowBandKey),
    ...
    // principal_frame: layout e1x, e1y, e2x, e2y per pixel
    const principalFrame = principalFrameArt.data?.frame
                           ?? principalFrameArt.data?.principalFrame
                           ?? null;
    if (!principalFrame) throw new Error('principal_frame artifact missing frame data');

    // flow_field: confirmed layout art.data.u / art.data.v
    const flowU = flowFieldArt.data?.u ?? null;
    const flowV = flowFieldArt.data?.v ?? null;
    if (!flowU || !flowV) throw new Error('flow_field artifact missing u/v components');

    // motion_maps: motionMagnitude (Float32Array) + motionEndsMap (Int32Array)
    const motionMagnitude = motionMapsArt.data?.motionMagnitude ?? null;
    const motionEndsMapRaw = motionMapsArt.data?.motionEndsMap  ?? null;
    const motionEndsMap    = motionEndsMapRaw
      ? new Int32Array(motionEndsMapRaw)
      : null;

    if (!motionMagnitude) {
      console.warn('[kem.worker] motionMagnitude absent — KEM magnitudes will be zero');
    }
    if (!motionEndsMap) {
      console.warn('[kem.worker] motionEndsMap absent — clade assignment will use background only');
    }

    // ── Unpack Group B ─────────────────────────────────────────────────────

    // coherencePerPixel — defensive extraction matching ambi.worker pattern
    const coherencePerPixel = _extractCoherence(directionalArt);
    if (!coherencePerPixel) {
      console.warn('[kem.worker] coherence.perPixel absent — reliability weighting defaults to 0.5');
    }

    // narrowBandMask — from phi_min artifact (phiMin artifact stores phi, not mask)
    // Reconstruct from |phi| < threshold if artifact present, else full-field fallback
    let narrowBandMask = null;
    if (phiMinArt?.data?.phi) {
      const phi    = phiMinArt.data.phi;
      const thresh = 12 / resolution;   // ~6 pixels at 512²
      narrowBandMask = new Float32Array(N);
      for (let i = 0; i < N; i++) {
        narrowBandMask[i] = Math.abs(phi[i]) < thresh ? 1 : 0;
      }
    } else {
      // No phi available — treat full image as narrow band (degraded)
      console.warn('[kem.worker] phi_min artifact absent — using full-image narrow band');
      narrowBandMask = new Float32Array(N).fill(1);
    }

    // Stage 5 surface param — for degradedMode and legibilityScore metadata
    const stage5Degraded      = surfaceParamArt?.data?.degradedMode   ?? false;
    const stage5Legibility    = surfaceParamArt?.data?.legibilityScore ?? 1.0;
